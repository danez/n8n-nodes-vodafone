...
const OIDC_SCOPES = 'openid profile webseal user-groups user-accounts validate-token update-email-username account';
const OIDC_CODE_CHALLENGE_LENGTH = 43;
const VODAFONE_API_KEY = 'aEIoMCae0A933wBL0bLlS6SwSBfkKwM5';
const USER_INFO_URL = 'https://api.vodafone.de/meinvodafone/v2/tmf-api/openid/v4/userinfo';
...
function parseSetCookieHeaders(headers: IDataObject): Array<{ name: string; value: string }> {
...
function mergeCookies(cookies: Map<string, string>, response: IN8nHttpFullResponse): Map<string, string> {
...
async function requestJson<T>(
	...
	if (response.statusCode !== 200) {
		throw createApiError(executeFunctions, `Vodafone API request failed while ${requestName}`, itemIndex, response);
	}
...
async function getInitialMintCookies(
	...
	if (response.statusCode !== 302) {
		throw createApiError(executeFunctions, 'Could not start Vodafone login session', itemIndex, response);
	}
...
async function exchangeCodeForToken(
	...
	if (response.statusCode !== 200) {
		throw createApiError(executeFunctions, 'Could not exchange Vodafone login code for a token', itemIndex, response);
	}

	const token = response.body as Partial<OidcTokenResponse>;

	if (!token.access_token || !token.token_type) {
		throw new NodeOperationError(executeFunctions.getNode(), 'Vodafone login response did not include an access token', {
			itemIndex,
		});
	}
...
export async function testVodafoneCredentials(
	...
	try {
		...
		const code = location ? new URL(location, OIDC_REDIRECT_URI).searchParams.get('code') : '';

		if (!code) {
			throw new Error('Vodafone login response did not include an authorization code');
		}
		...
		return {
			status: 'Error',
			message: error instanceof Error ? error.message : 'Vodafone credential test failed',
		};
	}
}
...
export async function login(
	...
	if (loginResponse.statusCode !== 200) {
		throw createApiError(executeFunctions, 'Vodafone login failed', itemIndex, loginResponse);
	}
	...
	if (authorizeResponse.statusCode !== 302) {
		throw createApiError(executeFunctions, 'Vodafone login did not return an authorization code', itemIndex, authorizeResponse);
	}
	...
	if (!location) {
		throw new NodeOperationError(executeFunctions.getNode(), 'Vodafone login response did not include a redirect location', {
			itemIndex,
		});
	}
	...
	if (!code) {
		throw new NodeOperationError(executeFunctions.getNode(), 'Vodafone login response did not include an authorization code', {
			itemIndex,
		});
	}
	...
	return {
		cookies,
		token: await exchangeCodeForToken(executeFunctions, code, verifier, itemIndex),
	};
}
...
export async function getUserInfo(
	...
	return {
		userAccountVBO: {
			cable: userInfo.flatMap((user) => (user.userAssets ?? []).flatMap(toCableAccount)),
		},
	};
}
...
function toCableAccount(asset: VodafoneUserAsset): VodafoneCableAccount[] {
	...
	const customerNumber =
		...
		findExternalIdentifier(asset.relatedAsset?.flatMap((relatedAsset) => relatedAsset.externalIdentifier ?? []), 'accountNumber') ??
	...
	const relatedBillingAccount = asset.relatedAsset?.find((relatedAsset) => relatedAsset.entityType === 'billingAccount');
	const characteristics = [...(asset.characteristic ?? []), ...(relatedBillingAccount?.characteristic ?? [])];
	...
	return [
		{
			...
			isDefaultContract: characteristicValue(characteristics, 'isDefault') === 'true',
			hasCableMail: characteristicValue(characteristics, 'hasCableMail') === 'true',
			subscription: [
				{
					id: findExternalIdentifier(asset.externalIdentifier, 'subscriptionId'),
...
function isCableAsset(asset: VodafoneUserAsset): boolean {
	return (
		...
		(asset.assetType === 'Broadband' || asset.id?.includes(':cable:') === true) &&
		(asset.characteristic?.some((characteristic) => characteristic.name === 'stack' && characteristic.value === 'Cable') === true ||
