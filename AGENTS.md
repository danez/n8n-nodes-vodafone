...
- This package implements a Vodafone Germany invoice downloader node, not the
  starter GitHub example.
- Use `pnpm with current ...` for local commands, for example
  `pnpm with current run build`, `pnpm with current run lint`, and
  `pnpm with current exec tsc --noEmit`.
- The node is programmatic because Vodafone requires login/OIDC session
  handling, contract discovery, invoice listing, document traversal, and binary
  output.
- Current contract discovery uses
  `https://api.vodafone.de/meinvodafone/v2/tmf-api/openid/v4/userinfo`.
- The older reference endpoint
  `https://api.vodafone.de/meinvodafone/v2/user/userInfo` returned 404 during
  testing and should not be used unless reverified.
- Invoice count means latest available invoice PDFs per contract. Skip newer
  invoice rows that have no document/PDF yet.
- Output PDFs are always written to binary property `data`.
- Do not log credentials, tokens, cookies, or invoice document payloads.
